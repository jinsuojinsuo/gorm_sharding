...
// groupDeleteValues 按分表字段把批量 Delete 的实体切片分组到对应真实分表。
// grouped 为 false 表示当前 Delete 不是非空切片或数组，应继续使用普通条件路由。
func (p *Plugin) groupDeleteValues(db *gorm.DB, cfg ShardingConfig) (groups map[string]reflect.Value, grouped bool, err error) {
...
// deleteShardValues 删除一个真实分表中的一组实体，并保留调用方已有的 WHERE、Select、Unscoped 等状态。
func (p *Plugin) deleteShardValues(db *gorm.DB, table string, values interface{}) *gorm.DB {
